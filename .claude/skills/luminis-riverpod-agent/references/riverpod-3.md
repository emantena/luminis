...
## Retry Automatico (Riverpod 3.x)

Descoberto ao implementar `books`/`bookshelf` (Onda 2): Riverpod 3.x adicionou
retry automatico com backoff exponencial para qualquer erro nao tratado que
escape do `build()`/computacao de um provider (`FutureProvider`,
`AsyncNotifierProvider`, `StreamProvider` e variantes `family`/`autoDispose`).

A logica (`ProviderContainer.defaultRetry`, em `package:riverpod`) tenta
novamente automaticamente qualquer erro que **nao** seja `Error` nem
`ProviderException` — ou seja, qualquer `Exception` comum, incluindo
`ApiFailure` (`shared/infrastructure/api_exception.dart`), e retenta ate 10
vezes com atraso exponencial (200ms a 6.4s). Isso nao aparece em nenhum
exemplo de `references/riverpod-3.md` anterior a esta revisao porque o
comportamento e novo na familia 3.x e nao existia em Riverpod 2.x.

Consequencias praticas:
- Um `ApiFailure` lancado dentro de `build()` (ex.: `BookshelfController.build`
  chamando `repository.listItems()`) nao vira `AsyncError` imediatamente — o
  provider fica "preso" retentando silenciosamente por varios segundos antes
  de finalmente expor o erro, o que atrasa (e em testes sincronos pode nunca
  chegar a expor, se o container for descartado antes) o estado de erro que
  a UI precisa mostrar.
- Isso NAO afeta comandos de `Notifier`/`NotifierProvider` sincronos que
  capturam `ApiFailure` internamente com `try/catch` (padrao usado em
  `LoginController`, `BookSearchController` etc.) — o retry so atua quando a
  excecao escapa do proprio `build()`/computacao do provider sem ser
  capturada.

Padrao adotado: todo `FutureProvider`/`AsyncNotifierProvider` (e variantes
`family`/`autoDispose`) cujo `build()`/computacao dependa de um repository
desta app deve passar `retry: noRetry`, usando o helper
...
Exemplos aplicados: `features/bookshelf/presentation/controllers/bookshelf_controller.dart`
e `features/books/presentation/controllers/book_detail_controller.dart`.
